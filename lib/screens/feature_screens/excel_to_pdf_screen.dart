...
class _ExcelToPdfScreenState extends State<ExcelToPdfScreen>
    ...
    try {
      ...
      for (var tableName in excelFile.tables.keys) {
        ...
        if (processedRows.isNotEmpty) {
          await _addSheetToPdf(pdf, tableName, processedRows);
        }
  ...
  Future<void> _addSheetToPdf(
      pw.Document pdf, String sheetName, List<List<String>> rows) async {
                    ...
                    children: chunk.map((row) {
                      ...
                      return pw.TableRow(
                        decoration: isHeaderRow
                            ...
                            : null,
                        children:
                            _buildTableCells(row, maxColumns, isHeaderRow),
                      );
                    }).toList(),
                  ),
                ),
  ...
  List<pw.Widget> _buildTableCells(
      List<String> row, int maxColumns, bool isHeader) {
